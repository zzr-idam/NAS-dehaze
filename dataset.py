...
class StyleDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        if content.shape[0] < content.shape[1]:
            ...
            content = content.transpose((2, 1, 0)) / 255.0

        if information.shape[0] < information.shape[1]:
            information = information.transpose((2, 0, 1)) / 255.0
        else:
            information = information.transpose((2, 1, 0)) / 255.0
        '''
...
class StyleDataset_test(Dataset):
    def __init__(self, content1, information, size):
        self.content1_list = content1
...
def style_loader_test(content1_folder, information_folder, size, batch_size):
    content_list1 = load_simple_list(content1_folder)

    information_list = load_simple_list(information_folder)

    dataset = StyleDataset_test(content_list1, information_list, size)
    num_workers = 8 if batch_size > 8 else batch_size
    dataloader = DataLoader(dataset, batch_size=batch_size,
